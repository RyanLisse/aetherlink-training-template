...
| 13:15–14:00 | 45 | Individual + group | Repeat with second scope |
...
**Prompt.** Run the Claude Code starter against the same local repository snapshot. Record successful observed tool calls, findings, exact paths, and `OPEN` GitLab or CI checks. Do not create remote records.

**Checkpoint.** A fresh reader can explain what the agent read, produced, and what remains unproven.
